...
const Register = () => {
    ...
    const handleSubmit = (e) => {
        ...
        data.append('nama_pelanggan', name);
        data.append('foto', image);
        data.append('email', email);
        data.append('password', password);

        axios.post('http://localhost:8000/hotel/pelanggan/', data)
            .then(res => {
                if (res.data.message === "Data has been inserted") {
                    setTimeout(() => {
                        navigate('/login')
                    }, 2000)
                }
                    ...
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit} encType="multipart/form-data">
                            ...
                            <div>
                                ...
                                <input type="file" name="image" id="image" onChange={(e) => setImage(e.target.files[0])} accept="image/*" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                            </div>
                            <div>
                            ...
                            <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                Have an account? <NavLink to="/login" className="font-medium text-blue-600 hover:underline dark:text-blue-500">Log in</NavLink>
                            </p>
                        </form>
                    </div>
                </div>
