...
const Login = () => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('http://localhost:8000/hotel/pelanggan/login', user)
            .then(res => {
                localStorage.setItem('token', res.data.token)
                localStorage.setItem('user', res.data.data.id_pelanggan)
                setTimeout(() => {
                    navigate('/dashboard')
                }, 2000)
            })
                    ...
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit} >
                            ...
                            <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                Don’t have an account yet? <NavLink to="/register" className="font-medium text-blue-600 hover:underline dark:text-blue-500">Sign up</NavLink>
                            </p>
                        </form>
                    </div>
                </div>
